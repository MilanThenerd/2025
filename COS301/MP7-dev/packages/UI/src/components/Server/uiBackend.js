...
const useCollectionManagement = () => {
  ...
  const fetchDatabases = useCallback(async () => {
    try {
      return await api.getAllDatabase();
    } catch (err) {
  ...
  const insertDocument = async (dbName, collectionName, document) => {
    try {
      return await api.createDocument(dbName, collectionName, {document:document});
    } catch (err) {
  ...
  const deleteDocument = async (dbName, collectionName, docId) => {
    try {
      return await api.deleteDocument(dbName, collectionName, "_id",docId);
    } catch (err) {
      throw new Error("Failed to delete document: " + err.message);
    }
  };

  const executeQuery = async (dbName, collectionName, field , value , operator) => {
    try {
      return await api.getDocument(dbName, collectionName, field , value , operator);
    } catch (err) {
  ...
  useEffect(() => {
    const loadCollections = async () => {
      ...
      try {
        ...
        // Reset selected collection if it doesn't exist in the new list
        if (selectedCollection && !collectionList.some(c => c.name === selectedCollection)) {
          setSelectedCollection(null);
        }
  ...
  useEffect(() => {
    const loadCollectionData = async () => {
      ...
      try {
        ...
        setCollectionData(Array.isArray(data) ? data : data.data || []);
        setError(null);
      } catch (err) {
        setError(err.message);
        setCollectionData([]);
...
const handleDeleteCollection = async () => {
  ...
  try {
    ...
    // Select first available collection or clear selection
    setSelectedCollection(
      updatedCollections.length > 0 
        ? updatedCollections[0].name 
        : null
    );
  ...
  const handleDeleteRow = async (rowToDelete) => {
    try {
      setLoading(true);
      await deleteDocument(selectedDatabase, selectedCollection, rowToDelete._id);
      setCollectionData(collectionData.filter(row => row.id !== rowToDelete._id));
      setError(null);
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const handleExecuteQuery = async (dbName, collectionName, field , value , operator) => {
    try {
      setLoading(true);
      const results = await executeQuery(dbName, collectionName, field , value , operator);
      return results;
    } catch (err) {
      setError(err.message);
      throw err;
